...
import { EditDevolutionForm } from "./edit-devolution-form";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function DevolutionApproval({
  ...
  >({});
  const [editingManualReturn, setEditingManualReturn] =
    useState<ManualReturn | null>(null);
  ...
  const canApproveReject =
    userRole === "PHARMACY_REGENT" || userRole === "SUPERADMIN";

  // Filter standalone manual returns that are pending
  const pendingStandaloneReturns = manualReturns.filter(
    (mr) => mr.type === "STANDALONE" && mr.status === ManualReturnStatus.PENDING
  );
  ...
  const handleEdit = (manualReturn: ManualReturn) => {
    setEditingManualReturn(manualReturn);
    setIsEditModalOpen(true);
  };

  const handleEditSuccess = () => {
    setIsEditModalOpen(false);
    setEditingManualReturn(null);
    onApprovalChange?.();
  };
  ...
  if (pendingStandaloneReturns.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            <Check className="h-5 w-5 text-green-600" />
            <span>Devoluciones Independientes</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">
            No hay devoluciones independientes pendientes de aprobación.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            ...
            <span>Devoluciones Independientes Pendientes</span>
            <Badge
              variant="outline"
              className="text-orange-600 border-orange-600"
            >
              {pendingStandaloneReturns.length}
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
            <p className="text-sm text-orange-800">
              ...
              independientes necesitan su aprobación antes de proceder.
            </p>
          </div>

          {pendingStandaloneReturns.map((manualReturn) => (
            <div
              key={manualReturn.id}
                ...
                <div className="flex flex-col space-y-3 pt-3 border-t">
                  <div className="flex space-x-2">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleEdit(manualReturn)}
                      disabled={
                        approveManualReturn.isPending ||
                ...
                <div className="pt-3 border-t">
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <p className="text-sm text-blue-800">
                      <strong>Información:</strong> Esta devolución
                      independiente está pendiente de aprobación por parte del
                      regente de farmacia.
                    </p>
                  </div>
                </div>
      ...
      {/* Edit Devolution Modal */}
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Editar y Aprobar Devolución Independiente</DialogTitle>
          </DialogHeader>
          {editingManualReturn && (
            <EditDevolutionForm
              manualReturn={editingManualReturn}
