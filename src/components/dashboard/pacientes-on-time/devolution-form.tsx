...
interface DevolutionFormProps {
  patientId: string;
  medicationProcessId?: string; // Optional - for standalone manual returns
  onSuccess: () => void;
  onCancel: () => void;
}
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await createManualReturn.mutateAsync({
        patientId,
        medicationProcessId,
        type: medicationProcessId
          ? ManualReturnType.DEVOLUTION_PROCESS
          : ManualReturnType.STANDALONE,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-2 text-blue-600">
        ...
        <h3 className="text-lg font-semibold">Nueva Devolución Manual</h3>
      </div>

      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          {medicationProcessId
            ? "Esta devolución será parte del proceso de devolución del paciente y requerirá aprobación del regente de farmacia antes de continuar."
            : "Esta es una devolución manual independiente que requerirá aprobación del regente de farmacia."}
        </p>
      </div>
